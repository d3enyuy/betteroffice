...
| `@betteroffice/pptx` | Framework-free PPTX core: PresentationML parse, deck model, masters and layouts, slide display lists, canvas rendering. Editing, comments, collaboration, and saving are live; untouched slides keep their exact source part bytes on save. |
...
## Migrate from eigenpal

Use `@betteroffice/docx` for the framework-free core and
`@betteroffice/docx-react` for the React editor. Update imports to those package
names and use the examples above to integrate BetterOffice. React and
framework-free JavaScript are the available entry points; there is no
`@betteroffice/docx-vue` package.
